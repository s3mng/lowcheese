...
import com.example.lowcheese.chzzk.DashParser
import com.example.lowcheese.chzzk.HlsParser
import com.example.lowcheese.model.QualityOption
import com.example.lowcheese.model.StreamProtocol
import com.example.lowcheese.net.download
import com.example.lowcheese.net.getText
import java.io.File
    ...
    private suspend fun downloadHls(
        ...
        output.use { sink ->
            if (playlist.mapUri != null) {
                ...
                http.download(playlist.mapUri, sink)
                done++
            ...
            for (segment in playlist.segments) {
                ...
                waitIfPaused(isPaused, isCancelled)
                coroutineContext.ensureActive()
                http.download(segment.uri, sink)
                done++
                onProgress(done / total.toFloat())
            }
        }
